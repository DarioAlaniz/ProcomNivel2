...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if(self.kernel == 1):   
            ...
            (_,countours,_) = cv2.findContours(canny.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv2.drawContours(img,countours,-1,(255,0,0), 1)
        elif(self.kernel == 2):
            _, thrash   = cv2.threshold(gray,240,255,cv2.THRESH_BINARY)
            _,countours,_ = cv2.findContours(thrash,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
        ...
        new_frame.pts = frame.pts #conserva el orden de los frame
...
async def javascript(request):
    ...
    return web.Response(content_type="application/javascript", text=content)#manda una solicitud al cliente
